...
You will also learn to use the _forcats_ package that is necessary for ordering plotting of categorical variable
...
by default `geom_smooth` is using a loess regression (< 1,000 points) and add standard error intervals. 

- The `method` argument can be used to change the regression to linear: `method = "lm"`
...
(@) Ajust some aesthetics in order to:
    ...
    + associate a color gradient to the quarter mile time
    + If you're feeling succesful, try to adjust the color gradient from yellow for fast accelerating cars to blue for slow accelerating cars.
...
```{asis,  boxtitle = "Tip", box = "lightblue"}
For the `cyl` variable, the data is set as double, so a continuous variable. To map as the shape aesthetics, 
mind coercing the variable to a factor
```
...
```{asis,  boxtitle = "Tip", box = "lightblue"}
Remember that aesthetic define in the `ggplot(aes())` command are passed on to all following layers.
While, `aes()` of individual geoms are specific (and overwrite the global if present).
```


## Categorical data

We now will be looking at another built-in dataset `ToothGrowth`. Contains the tooth length of 60 guinear pigs
after they received 3 different doses of vitamin C (in mg/day), 
provided either by orange juice (`OJ`) or ascorbic acide (`VC`) 
...
(@) plot the distributions as boxplots  of the tooth' lengths by the dose received
...
(@) add the supplement type to the previous plot, as a filling colour
...
When the dataset is tidy, we can easily perform a plot that tells the story: vitamin C does affect the tooth growth
and the delivery methods only matter for lower concentrations.

---

Boxplots are nice but misleading. Data size are absent, and the distibution shape could be better representated.

(@) plot again the tooth' lengths by the dose received using `geom_dotplot()`
...
(@) add the `geom_violin()` to the previous to get a better view of the distribution shape

```{asis,  boxtitle = "Tip", box = "lightblue"}
the layer' order matters. Plotting is done respectively. 
...
Now we are missing any summary value, like the median shown by the boxplots. We should add one.

(@) add to the previous plot the median using `stat_summary()`

```{asis,  boxtitle = "Tip", box = "lightblue"}
by default stat_summary add the mean and +/- standard error via geom_pointrange().
...
(@) change the `stat_summary()` in the previous plot from **median** to **mean_cl_boot** and polish labels

different summary statistics from the library `Hmisc` are available. Let's try the **mean_cl_boot** that compute
the non-parametric bootstrap for obtaining 95% confidence intervals (`mean_cl_normal` assumes normality)
...
Of note, a `ggplot` extension named `ggbeeswarm` proposed a very neat dotplot that fits the distribution.
...
Let's now look at a real output file for CNV detection. The tool used is called **Reference Coverage Profiles**: [RCP](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4330915/). It was developed by  
analyzing the depth of coverage in over 6000 high quality (>40×) genomes. In the end, for every _kb_ a state is assigned 
...
The file is accessible [here](https://lsru.github.io/tv_course/practicals/CNV.seg.gz). 
It is gzipped but `readr` will take care of the decompression. Actually, `readr` could also read the file from the 
website so you don't need to download locally.

`CNV.seg.gz` has 5 columns and the first 10 lines looks like:
...
- chromosome will be read as integers since first 1000 lines are **1**. But, X, Y are at the file' end.
- first and last column names are unclean. `#chrom` comtains a hash and `length (kb)`. Would be neater to fix this upfront.
```

```{r, solution = TRUE}
cnv <- read_tsv("CNV.seg.gz", skip = 2,
...
(@) plot the counts of the different states. We expect a majority of diploid state.
...
(@) using the previous plot, reorder the levels of chromosome to have the karyotype order (1:22, X, Y)

```{asis,  boxtitle = "Tip", box = "lightblue"}
we could explicity provide the full levels lists in the desired order. However, in the tibble, the chromosomes appear 
in the wanted order. See the `fct_inorder()` function in the **forcats** package to take advantage of this.
```
...
```{asis,  boxtitle = "Tip", box = "lightblue"}
distributions are absolutely skewed. Transfrom to log-scale to get a decent plot.
...
```{r, solution = TRUE, fig.height=12}
cnv_auto %>%
  mutate(state = if_else(state == 1, "loss", "gain")) %>%
  mutate(chr = forcats::fct_inorder(chr)) %>%
  ggplot(aes(x = start, xend = end, y = state, yend = state, colour = as.factor(state))) +
  scale_x_continuous(expand = c(0, 0), breaks = c(0, 100e6, 200e6), labels = c(1, 100, 200)) +
