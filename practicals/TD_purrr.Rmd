...
list.files(file.path("data", "pcr"), full.names = TRUE) %>%
  ...
  names()
```
6. Getting a single `tibble` out of all files would be much handier. Instead of further transforming this output we will use another member of the `map()` family of functions that will immediately create the desired output. Change your previous code to get a data frame directly.
...
dir.create(file.path("data", "by_gene"))
# Construct the Use paste0 with this path
```
...
list.files(file.path("data", "pcr"), full.names = TRUE) %>%
  ...
  mutate(file_out = paste0(gene, ".csv"),
         file_out_path = paste0(file.path("data", "by_gene", file_out), ".csv"),
         walk2(data, file_out_path, write_csv))
```
...
```{r, echo = FALSE}
dir.create(file.path("data", "plots"))
```
...
list.files(file.path("data", "pcr"), full.names = TRUE) %>%
  ...
  group_by(filename) %>%
  nest() %>%
  mutate(plot_file = basename(filename) %>% stringr::str_replace(".csv", ".pdf"),
         plot_file_path = file.path("data", "plots", plot_file),
         plot = map(data, ~ggplot(data = ., aes(x = type, y = Ct)) + geom_boxplot()),
         walk2(plot_file_path, plot, ggsave, device = "pdf"))
```
