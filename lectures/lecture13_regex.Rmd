...
### A first glance at regular expressions {.box-6 .bg-blue}
 * Matching substituting of character strings 
 * Ugly, unreadable, terrifying expression s/^lecture([0-9]{1,2}).*[^_].Rmd$/\1.Rmd/
 * See [R for data science](http://r4ds.had.co.nz/strings.html#matching-patterns-with-regular-expressions)
 * Slides are adapted from presentation "LECTURE 16: REGULAR EXPRESSIONS IN R" by Vinayak Rao
 
### The only important advice  {.box-6 .bg-red}
...
### `sub, gsub` {.box-6 .bg-greenwhite} Search a character vector for regular expression matches and replace that match with another string
...
## Meta characters
### Covered so far {.box-8 .bg-bluewhite}
 * `.` 
 * `,` 
 * `+` 
 * `*`  
 
### Other useful ones include {.box-8 .bg-bluewhite}
 * `^` Start of line
 * `$` End of line
```{r}
vec
